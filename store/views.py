...
import json
import datetime
from .models import Customer, Product, Order, OrderItem, ShippingAddress, Review  # noqa
...
class ProductList(generic.ListView):
    '''
    Displays all products from :model:`store.Product`
    using djangos list view with pagination.

    **Context**

    ``items``
        The items for any open carts to calculate cart totals
        in the navbar in this view from :model:`OrderItem`

    ``order``
        The users current order/open cart object from :model:`store.Order`

    ``cartItems``
        Total number of cart items for the
        asket total from :model:`store.Order`
    ...
    # Override class context method to pass cart
    # total as cartItems to the view template
...
def cart(request):
    '''
    Fetches or creates a single :model: `store.Order` instance
    for the current customer dynamically
    by logged in status and sets it's items before rendering it

    **Context**

    ``items``
        All items with this order ID to be attached
        to the order from :model: `OrderItem`

    ``order``
        The users current order/open cart
        object from :model: `store.Order`

    ``cartItems``
        Total number of cart items for the
        basket total from :model: `store.Order`
...
def product_detail(request, slug):
    ...
    **Context**
    ...
    ``cartItems``
        Total number of cart items for the
        basket total from :model:`store.OrderItem`
...
def checkout(request):
    ...
    **Context**
    ...
    ``items``
        All items from :model:`OrderItems` related
        to thie :model:`store.Order` instance.

    ``cartItems``
        Total number of cart items for the basket
        total from :model:`store.OrderItem`
...
def updateItem(request):
    """
    Updates the instance of :model:`store.Order`
    for displaying the cart and checkout pages.
    """
...
def processOrder(request):
    ...
    if request.user.is_authenticated:
        ...
        # Check cart items haven't been manipulated in front end
        # with JS before setting order to complete
        # Below only works with guest cart implemented.
        # Commented out to implement after marking.
        # if total == order.get_cart_total:
        #    order.complete = True

        # Can be spoofed by hackers until I finish guest checkout
        # but close order as completed for now
        order.complete = True
        order.save()
